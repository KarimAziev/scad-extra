...
- =scad-extra-copy-plist-with-format=
  Select complete property/value pairs inside an OpenSCAD plist and run this
  command (or press =p= in =scad-extra-menu=).  Choose a format while previewing
  the transformed selection, then enter the plist variable name; pressing
  =RET= accepts =plist=.  The result is copied to the kill ring for yanking
  elsewhere.  For example, selecting:
  ...
  and choosing the first format copies:
  ...
  A whole bracketed plist can also be selected, or point can be placed at its
  opening =[= without a region.  Comments between entries are skipped, and
  nested lists remain values.  Customize =scad-extra-plist-copy-formats= to
  change the choices: =%k= is the unquoted key, =%K= the original quoted key,
  =%v= the value, and =%p= the plist variable name.  The supplied formats also
  support using original values as =plist_get= defaults or copying ordinary
